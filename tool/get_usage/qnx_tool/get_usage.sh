...
make_data_fixed_length()
{
	...
	cnt=`echo "$MAX_ITERATION-$COUNT" | bc`
	...
	while [[ $cnt -gt 0 ]]; do
		cnt=$((cnt-1))
		task_nr=$MAX_TASK
		while [[  $task_nr -gt 0 ]]; do
			task_nr=$((task_nr-1))
			echo "N/A,N/A,N/A,N/A,N/A,N/A" >> $PATH_OUTPUT_DIR/all_data.csv
		done
	done
}
...
make_iteration_fixed_length()
{
	max=60

	# add blank line for fixed length
	filenr=`find *.000 | wc -l`
	cnt=`echo "$max-$filenr" | bc`
	if [ 1 -gt "$cnt" ]; then
		continue;
	fi
